...
Sweep *sweep, *sweep2;
Mesh *mesh;
Shader *shade, *depthShade, *blurShade;
...
void blurShadowMap(void)
{
	...
	// Bluring the shadow map  horinzontaly
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,blurFboId);	
		glViewport(0,0,viewport.w * SHADOW_MAP_COEF *BLUR_COEF ,viewport.h* SHADOW_MAP_COEF*BLUR_COEF);
		glUseProgramObjectARB(blurShade->getProgram());
		//glUniform2fARB(blurShade->getScaleAttrib(),1.0/ (viewport.w * SHADOW_MAP_COEF * BLUR_COEF),0.0);		// Bluring horinzontaly
		//glUniform1iARB(blurShade->getTextureSourceAttrib(),0);

		blurShade->setScale(1.0/ (viewport.w * SHADOW_MAP_COEF * BLUR_COEF),0.0);
		blurShade->setTextureSource(0);
		...
		//glGenerateMipmapEXT(GL_TEXTURE_2D);
		 ...
		 // Bluring vertically
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,fboId);	
		glViewport(0,0,viewport.w * SHADOW_MAP_COEF ,viewport.h* SHADOW_MAP_COEF);
		//glUniform2fARB(blurShade->getScaleAttrib(),0.0, 1.0/ (viewport.h * SHADOW_MAP_COEF ) );
		blurShade->setScale(0.0,1.0/ (viewport.h * SHADOW_MAP_COEF ) );		
		glBindTexture(GL_TEXTURE_2D,blurFboIdColorTextureId);
		glBegin(GL_QUADS);
			 glTexCoord2d(0,0);glVertex3f(-viewport.w/2,-viewport.h/2,0);
			 glTexCoord2d(1,0);glVertex3f(viewport.w/2,-viewport.h/2,0);
...
/*
//Camera position
vec3 p_camera(35,25,5);

//Camera lookAt
vec3 l_camera(0,0,-10);
...
void drawStuff(int type, Shader * stupid) {
  ...
  mesh->draw(*shade);
  if(type == 0)
    sweep->renderWithDisplayList(*stupid,20,0.3,20);
  else 
    sweep2->renderWithDisplayList(*stupid,20,0.3,20);
  endTranslate();
}
...
//First step: Render from the light POV to a FBO, store depth and square depth in a 32F frameBuffer
  ...
  glViewport(0,0,viewport.w * SHADOW_MAP_RATIO,viewport.h* SHADOW_MAP_RATIO);
	...
	/*
	 ...
	 glBegin(GL_QUADS);
	 glTexCoord2d(0,0);glVertex3f(0,0,0);
	 glTexCoord2d(1,0);glVertex3f(viewport.h/2,0,0);
	 glTexCoord2d(1,1);glVertex3f(viewport.w/2,viewport.h/2,0);
	 glTexCoord2d(0,1);glVertex3f(0,viewport.h/2,0);
	 glEnd();
	 glDisable(GL_TEXTURE_2D);
...
*/
	...
	glUseProgramObjectARB(shade->getProgram());
	glUniform1iARB(shade->getShadowMapAttrib(),4);
	//glUniform1fARB(shade->getXPixelOffsetAttrib(),1.0/ (viewport.w * SHADOW_MAP_COEF));
        //glUniform1fARB(shade->getYPixelOffsetAttrib(),1.0/ (viewport.h * SHADOW_MAP_COEF));
	glActiveTextureARB(GL_TEXTURE4);
	glBindTexture(GL_TEXTURE_2D,colorTextureId);
...
int main(int argc,char** argv) {
  ...
  blurShade = new Shader("gaussianBlurShader.vert", "gaussianBlurShader.frag");
  depthShade = new Shader("storeDepthShader.vert", "storeDepthShader.frag");
