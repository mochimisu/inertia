    ...
    for (int i = 1; i < size-1; i++) {
        ...
        if (i > 1) {
            ...
            for (int v = 0; v <= csize; v++) {
                int vn = v % csize;
                tan0 = oldSlice[(vn+1)%csize]-oldSlice[vn]; tan0.normalize();
                tan1 = newSlice[(vn+1)%csize]-newSlice[vn]; tan1.normalize();
				...
				coords[0] = (percent - 2*deltaPercent)*lengthRepeats;
				coords[1] = percentAround*widthRepeats;
				...
				b = (n^tan0);
				glVertexAttrib3dvARB(tangentAttrib, &tan0[0]);
				glVertexAttrib3dvARB(bitangentAttrib, &b[0]);
                ...
                // HINT: use percent, deltaPercent, percentAround to determine where on the curves you are
                // HINT: use lengthRepeats and widthRepeats to determine how much to repeat
				coords[0] = (percent - 1*deltaPercent)*lengthRepeats;
                ...
                // glVertexAttrib3dARBv(bitangentAttrib, x=?, y=?, z=?);
				b = (n2^tan1);
				glVertexAttrib3dvARB(tangentAttrib, &tan1[0]);
				glVertexAttrib3dvARB(bitangentAttrib, &b[0]);
				glTexCoord2dv(coords);
