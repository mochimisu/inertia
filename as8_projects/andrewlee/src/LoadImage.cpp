...
void computeNormalMapFromHeightMap(FIBITMAP *heights, FIBITMAP *normals, double zScale) {
    ...
    for (int xPrev = w-2, x = w-1, xNext = 0; xNext < w; xPrev=x, x=xNext++) {
        ...
        for (int yPrev = h-2, y = h-1, yNext = 0; yNext < h; yPrev=y, y=yNext++) {
			...
			vec3 tangentX = vec3(0.2, 0, pix(heights, xNext, y) - pix(heights, xPrev, y));
			vec3 tangentY = vec3(0, 0.2, pix(heights, x, yNext) - pix(heights, x, yPrev));
			vec3 normal = (tangentX^tangentY).normalize();

			//cout << normal << endl;

            color.rgbRed = BYTE((normal[0] + 1) * 127);
            color.rgbGreen = BYTE((normal[1] + 1) * 127);
            color.rgbBlue = BYTE((normal[2] + 1) * 127);
