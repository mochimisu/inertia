    ...
    for (int i = 1; i < size-1; i++) {
        ...
        if (i > 1) {
            ...
            for (int v = 0; v <= csize - 3; v++) {
				...
				double textureY = percentAround * widthRepeats;
				glTexCoord2f(percent * lengthRepeats, textureY);

                // @TODO: SET TANGENT AND BITANGENT
                // HINT: Use the glVertexAttrib3fARB, like:
                // glVertexAttrib3dARBv(bitangentAttrib, x=?, y=?, z=?);
				vec3 bitan0 = n^tan0;
                //glVertexAttrib3fARB(shader.getTangentAttrib(), tan0[VX], tan0[VY], tan0[VZ]);
                //glVertexAttrib3fARB(shader.getBitangentAttrib(), bitan0[VX], bitan0[VY], bitan0[VZ]);
                glVertexAttrib3fARB(shader.getBitangentAttrib(), tan0[VX], tan0[VY], tan0[VZ]);
                glVertexAttrib3fARB(shader.getTangentAttrib(), bitan0[VX], bitan0[VY], bitan0[VZ]);
                ...
                // HINT: use percent, deltaPercent, percentAround to determine where on the curves you are
                // HINT: use lengthRepeats and widthRepeats to determine how much to repeat
				glTexCoord2f((percent + deltaPercent) * lengthRepeats, textureY);
                ...
                // glVertexAttrib3dARBv(bitangentAttrib, x=?, y=?, z=?);
				vec3 bitan1 = n2^tan1;
                //glVertexAttrib3fARB(shader.getTangentAttrib(), tan1[VX], tan1[VY], tan1[VZ]);
                //glVertexAttrib3fARB(shader.getBitangentAttrib(), bitan1[VX], bitan1[VY], bitan1[VZ]);
                glVertexAttrib3fARB(shader.getBitangentAttrib(), tan1[VX], tan1[VY], tan1[VZ]);
                glVertexAttrib3fARB(shader.getTangentAttrib(), bitan1[VX], bitan1[VY], bitan1[VZ]);
...
/*
void Sweep::sampleWalk(double t, double s, vec3 &position, vec3 &forward, vec3 &up) {
	forward = sampleForward(t).normalize();
	vec3 tempUp = sampleUp(t);
	
    vec2 tempProf = sampleBSpline(profilePts, s);
	tempProf *= 1 + 0.5 / tempProf.length();
	vec3 prof = vec3(tempProf, 0);

	vec3 initialZ(0, 0, -1);
	mat4 rot1 = rotation3D(initialZ ^ forward, initialZ * forward * 360.0 / M_PI);
}*/

void Sweep::sampleWalk(double t, double s, vec3 &position, mat4 &matrix) {
	position = sample(t).point;
	vec3 forward = sampleForward(t).normalize();
	vec3 tempUp = sampleUp(t).normalize();
	...
	mat4 rot1 = rotation3D(initialZ ^ forward, acos(initialZ * forward) * 180.0 / M_PI);
	vec3 newY = rot1 * initialY;
	
	mat4 rot2 = rotation3D(newY ^ tempUp, acos(newY * tempUp) * 180.0 / M_PI);

	vec3 newUp = rot2 * rot1 * prof;
	
	mat4 rot3 = rotation3D(newY ^ newUp, acos(newY * newUp) * 180.0 / M_PI);
	...
	matrix = rotation3D(initZ ^ forward, acos(initZ * forward) * 180.0 / M_PI);
}